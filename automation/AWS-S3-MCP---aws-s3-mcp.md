...
Deploy and Host AWS S3 MCP with Railway
...
| --------- | ------- | ------- | ----------- |
| `PORT` | AWS S3 MCP Gateway | 80 | Port nginx listens on. Must match the domain's target port. |
| `API_KEYS` | AWS S3 MCP Gateway | (secret) | Comma-separated list of bearer tokens allowed to call the MCP. Allowed characters per key: A-Z a-z 0-9 . _ ~ + / = - |
| `MCP_HOST` | AWS S3 MCP Gateway | - | Hostname of the MCP service on Railway's private network. Only override if you rename the MCP service. |
| `MCP_PORT` | AWS S3 MCP Gateway | - | Port the MCP service listens on. Must match PORT on the MCP service. |
| `PATH_KEY_AUTH` | AWS S3 MCP Gateway | false | true also accepts the key as a path segment (/k/<key>/mcp) for MCP clients that cannot send an Authorization header. |
| `PORT` | AWS S3 MCP | 3000 | Port the MCP server binds to. Fixed at 3000 to match MCP_PORT on the gateway. |
| `AWS_REGION` | AWS S3 MCP | - | Region of the target buckets. auto for a Railway Bucket (reference its AWS_DEFAULT_REGION), an AWS region such as us-east-1 otherwise. |
...
| `AWS_ENDPOINT` | AWS S3 MCP | - | S3 endpoint URL for non-AWS storage. For a Railway Bucket, reference its AWS_ENDPOINT_URL. Leave empty for real AWS. |
| `S3_MAX_BUCKETS` | AWS S3 MCP | 5 | Maximum number of buckets the list-buckets tool returns. |
