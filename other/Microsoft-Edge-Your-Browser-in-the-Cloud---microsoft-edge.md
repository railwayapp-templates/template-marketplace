# Deploy Microsoft Edge | Your Browser in the Cloud on Railway

Desktop Microsoft Edge streamed to any device. Password-gated, persistent.

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/microsoft-edge)
...
The service streams a GPU-less desktop Microsoft Edge over WebSockets with the linuxserver.io Selkies stack. Access is gated by HTTP basic auth (`CUSTOM_USER` / generated `PASSWORD`), and the browser profile persists on a volume at `/config`. The `--disable-dev-shm-usage` flag is pre-set in `EDGE_CLI` so Microsoft Edge runs happily within Railway's container shared memory; append a URL there to open it at launch.

## What gets deployed

| Service | Source | Type |
|---------|--------|------|
| edge | [nomideusz/edge-railway](https://github.com/nomideusz/edge-railway) | Web service |
...
| `EDGE_CLI` | --disable-dev-shm-usage | Extra browser flags. --disable-dev-shm-usage is required on Railway (small /dev/shm). Append URLs or more flags space-separated. |
...
[View on Railway →](https://railway.com/deploy/microsoft-edge)
