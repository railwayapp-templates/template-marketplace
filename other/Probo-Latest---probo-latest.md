...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/probo-latest)

## About

Probo is open-source compliance management software for organizing controls, evidence, risks, vendors, policies, and audits.

This template deploys the latest Probo release tested by this template, currently v0.271.0, with PostgreSQL, a Railway Bucket, and headless Chrome. The Probo and Chrome images are pinned by immutable digest so every deployment is reproducible; the repository updates those pins only after the new release passes its deployment tests.
...
No OpenAI key is required for MCP-first operation through Codex. When the variable is left blank, the wrapper uses a clearly non-secret placeholder solely to satisfy Probo v0.271.0 startup wiring. The application and MCP remain available; Probo's built-in AI features require a real provider key.
...
[View on Railway →](https://railway.com/deploy/probo-latest)
