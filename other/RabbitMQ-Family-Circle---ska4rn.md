# Deploy RabbitMQ-Family-Circle on Railway
...
| Service | Source | Type |
|---------|--------|------|
| RabbitMQ | `rabbitmq:management-alpine` | Database |
...
- **Start command:** `/bin/sh -c "echo management.tcp.ip = :: >> /etc/rabbitmq/conf.d/10-defaults.conf && docker-entrypoint.sh rabbitmq-server"`
