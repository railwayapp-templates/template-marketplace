...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/redis-2)
...
`REDIS_PASSWORD` is required. The server is started with `--requirepass "$REDIS_PASSWORD"`, so Redis will not accept unauthenticated connections — set it as a generated secret in the Railway dashboard before deploying. Clients must authenticate with `AUTH ` (or `redis-cli -a `).
...
[View on Railway →](https://railway.com/deploy/redis-2)
