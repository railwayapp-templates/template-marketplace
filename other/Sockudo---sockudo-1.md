...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/sockudo-1)
...
| --------- | ------- | ------- | ----------- |
| `REDISHOST` | Redis | - | Private hostname of the included Redis service. Keep the Railway private-domain reference for connections within this project. |
| `REDISPORT` | Redis | 6379 | Redis connection port. Keep the supplied value to match the Redis service listener. |
| `REDISUSER` | Redis | default | Redis authentication username. Keep the supplied username for the included Redis service; change it only when configuring a matching Redis ACL user. |
| `REDIS_URL` | Redis | - | Connection string for connecting to redis using the private network |
| `REDISPASSWORD` | Redis | (secret) | Redis connection password, linked to this service's generated REDIS_PASSWORD. Keep the reference so client credentials remain consistent. |
| `REDIS_PASSWORD` | Redis | (secret) | Generated password securing the Redis service. Keep it private and preserve the matching client references when rotating it. |
...
[View on Railway →](https://railway.com/deploy/sockudo-1)
