...
| --------- | ------- | ----------- |
| `PORT` | 8502 | HTTP port Railway routes to. Railway injects this automatically. |
| `HERMES_HOME` | /opt/data | Root directory for all Hermes persistent state (boards, profiles, config). Mounted as a Railway volume. |
| `HERMES_KANBAN_API_TOKEN` | (secret) | OPTIONAL bearer token. When set, all API requests (including the SSE stream) require 'Authorization: Bearer <token>'. Leave empty to keep the board open. |
