...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/bisibility)
...
| worker | `corgicorner/bisibility-worker:0.4.1` | Worker |
| redis | `redis:8.2.1` | Database |
| temporal-postgres | `ghcr.io/railwayapp-templates/postgres-ssl:18` | Database |
| app-contract-cleanup | `corgicorner/bisibility:0.4.1` | Worker |
| temporal-schema | `temporalio/admin-tools:1.31.2` | Worker |
| web | `corgicorner/bisibility:0.4.1` | Web service |
...
| `SITE_URL` | app-contract-cleanup | - | Public application origin |
| `DIRECT_URL` | app-contract-cleanup | - | Migration database |
| `DATABASE_URL` | app-contract-cleanup | - | Application database |
| `DEPLOYMENT_ENV` | app-contract-cleanup | production | Deployment stage |
| `INTERNAL_PROBE_TOKEN` | app-contract-cleanup | (secret) | Detailed health probe token |
...
- **Start command:** `sh -eu -c 'deadline=$(( $(date +%s) + 600 )); until npm run public-id:contract-cleanup -- cleanup --health-url "$SITE_URL/api/v1/health" --expected-app-release "$APP_VERSION"; do if [ "$(date +%s)" -ge "$deadline" ]; then echo "Timed out waiting for the exact web release and public ID cleanup." >&2; exit 1; fi; sleep 5; done'`
...
[View on Railway →](https://railway.com/deploy/bisibility)
