...
| Service | Source | Type |
|---------|--------|------|
| Locomotive | `ghcr.io/brody192/locomotive:latest` | Worker |

## Environment variables

| Variable | Default | Description |
| --------- | ------- | ----------- |
| `LOCOMOTIVE_SERVICE_IDS` | - | Single Service ID or list of Service IDs. These will be the services to ship logs from.  Delimited by commas. |
| `LOCOMOTIVE_WEBHOOK_URL` | - | The destination of the logs. See the [overview](https://railway.com/deploy/locomotive) for platform examples. |
| `LOCOMOTIVE_WEBHOOK_MODE` | - | The mode to run Locomotive in defines the format used for sending the webhook to the target URL. See the [overview](https://railway.com/deploy/locomotive) for available modes. |
| `LOCOMOTIVE_ENVIRONMENT_ID` | - | By default, this is a reference to the environment ID that Locomotive is deployed into. |
| `LOCOMOTIVE_RAILWAY_API_KEY` | (secret) | Account scoped API token.  Project and Workspace tokens will not work |
| `LOCOMOTIVE_ENABLE_HTTP_LOGS` | false | Enable shipping HTTP logs; disabled by default. |
| `LOCOMOTIVE_ADDITIONAL_HEADERS` | - | A way to add authentication or any additional headers needed to the underlying HTTP request.  See the [overview](https://railway.com/deploy/locomotive) for usage examples. |
| `LOCOMOTIVE_ENABLE_DEPLOY_LOGS` | true | Enable shipping deploy logs; enabled by default. |
| `LOCOMOTIVE_REPORT_STATUS_EVERY` | 1m | How often Locomotive will report how many deploy and/or http logs it has shipped. |
