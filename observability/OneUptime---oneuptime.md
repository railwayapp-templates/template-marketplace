# Deploy OneUptime on Railway

Open-source monitoring, observability, incidents, and status pages

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/oneuptime)
...
| `POSTGRES_DB` | Postgres | railway | Default database created at startup. |
...
| `DATABASE_PUBLIC_URL` | Postgres | - | Public PostgreSQL connection URL with TLS. |
| `REDISHOST` | Redis | - | Private hostname of Redis. |
| `REDISPORT` | Redis | 6379 | Internal Redis port. |
| `REDISUSER` | Redis | default | Default Redis username. |
| `REDIS_URL` | Redis | - | Private authenticated Redis connection URL. |
| `REDISPASSWORD` | Redis | (secret) | Alias for the Redis password. |
| `REDIS_PASSWORD` | Redis | (secret) | Generated Redis password. |
| `REDIS_PUBLIC_URL` | Redis | - | Public authenticated Redis connection URL. |
...
[View on Railway →](https://railway.com/deploy/oneuptime)
